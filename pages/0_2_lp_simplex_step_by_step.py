...
def parse_lp(text_obj, text_cons):
    f = parse_expr(text_obj)

    constraints = []
    for raw in text_cons.split(","):
        raw = raw.strip()
        if "<=" in raw:
            lhs, rhs = raw.split("<=")
            constraints.append(parse_expr(lhs) - parse_expr(rhs))
        elif ">=" in raw:
            lhs, rhs = raw.split(">=")
            constraints.append(parse_expr(rhs) - parse_expr(lhs))
        else:
            constraints.append(parse_expr(raw))

    vars_ = sorted(
        set(f.free_symbols).union(*[c.free_symbols for c in constraints]),
        key=lambda s: s.name
    )
    ...
    for c in constraints:
        row = [float(c.coeff(v)) for v in vars_]
        rhs = float(-c.subs({v: 0 for v in vars_}))
        A.append(row)
        b.append(rhs)

    c = np.array([float(f.coeff(v)) for v in vars_])

    return vars_, np.array(A), np.array(b), c
...
def build_tableau(A, b, c):
    ...
    return pd.DataFrame(tableau, columns=cols, index=rows)


def choose_entering(T):
    row = T.loc["z"].drop("RHS")
    return row.idxmin() if row.min() < 0 else None


def choose_leaving(T, enter):
    ratios = {}
    for r in T.index[:-1]:
        a = T.loc[r, enter]
        if a > 0:
...
def pivot(T, enter, leave):
    ...
    p = T.loc[leave, enter]
    ops.append(f"R_{leave} ← R_{leave} / {p:.2f}")
    T.loc[leave] /= p
    ...
    for r in T.index:
        if r != leave:
            coef = T.loc[r, enter]
            if coef != 0:
                ops.append(f"R_{r} ← R_{r} − ({coef:.2f})·R_{leave}")
...
def main():
    ...
    st.title("📘 Linear Programming — Simplex Method (General)")
    ...
    selected = st.selectbox("📂 Load saved LP", ["—"] + files)
    ...
    if selected != "—" and st.button("Load"):
        ...
        prob = data["problem"]
        vars_, A, b, c = parse_lp(prob["objective"], prob["constraints"])

        # min → max
        c = -c

        st.session_state.vars = vars_
        st.session_state.tableau = build_tableau(A, b, c)
        st.session_state.history = []
        st.session_state.iter = 0

        st.success("LP loaded into Simplex")

    # --------------------------------------------------
    # Display tableau
    # --------------------------------------------------
    if st.session_state.tableau is None:
        st.info("Load an LP problem to start Simplex.")
        return
    ...
    # Simplex Step
    # --------------------------------------------------
    enter = choose_entering(st.session_state.tableau)

    if enter is None:
        st.success("Optimal solution reached 🎉")
    else:
        leave = choose_leaving(st.session_state.tableau, enter)

        st.markdown(
            f"""
            **Entering variable:** `{enter}`  
            **Leaving variable:** `{leave}`  
            **Pivot:** {st.session_state.tableau.loc[leave, enter]:.2f}
            """
        )

        if st.button("➡️ Next Simplex Step"):
            T_new, ops = pivot(st.session_state.tableau, enter, leave)
